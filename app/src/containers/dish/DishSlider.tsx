  ...
  if (source.uri?.includes('.glb')) {
    return (
      <ImageSliderRenderItem
        source={{uri: 'https://picsum.photos/300/262'}}
...
export const DishSlider = () => {
  ...
  const sources = compact([...imageUrls, imageUrls[0], modeUrl]).map(uri => ({
    uri,
  }));
  if (sources.length === 0) {
    return (
  ...
  return (
    <ImageSlider data={sources} itemHeight={262} RenderItem={DishRenderItem} />
  );
};
