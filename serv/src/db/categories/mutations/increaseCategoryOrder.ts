...
export default resolver.pipe(
  ...
  async (id, ctx) => {
    ...
    return db.$transaction([
      ...
      db.category.update({ where: { id }, data: { order: nextCat.order } }),
      db.category.update({ where: { id: nextCat.id }, data: { order: currentCat.order } }),

      // Negate orders of remaining categories
      ...categories
        .filter((v) => v.id !== id && v.id !== nextCat.id)
