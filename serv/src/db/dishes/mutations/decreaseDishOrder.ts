...
export default resolver.pipe(
  ...
  async (id, ctx) => {
    ...
    const prevDish = dishes[currentDishIndex - 1]
    if (currentDish == null) throw new Error("Cannot find current dish")

    // Mutate
    if (prevDish == null) return true
    return db.$transaction([
      ...
      db.dish.update({ where: { id }, data: { order: prevDish.order } }),
      db.dish.update({ where: { id: prevDish.id }, data: { order: currentDish.order } }),

      // Restore orders of remaining dishes
      ...dishes
        .filter((v) => v.id !== id && v.id !== prevDish.id)
